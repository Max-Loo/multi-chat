...
- **聊天存储架构重构**：重构聊天存储架构，支持按 key 存储、懒加载和导出功能
- **Shift+悬停快捷删除聊天**：添加 Shift+悬停快捷删除聊天功能
- **放宽聊天标题长度限制**：放宽聊天标题长度限制至 20 字符

### 🐛 Bug 修复

- **修复聊天名称过长时右侧按钮被压缩**：修复聊天名称过长时右侧按钮被压缩的问题
- **修复后台聊天内存泄漏**：修复后台聊天发送结束后 activeChatData 未正确释放的问题
- **修复 chunk 分类错误**：精确匹配包名，修复 pnpm 路径导致的 chunk 分类错误

### 🧪 测试增强

- **修复路由测试类型兼容性**：修复路由测试辅助函数与 react-router-dom 7.14.x 的类型兼容性
- **修复自动命名集成测试**：修复自动命名集成测试，适配后台聊天释放逻辑

### 🔧 杂项

- **更新翻译资源类型定义**：更新 i18n 翻译资源类型定义

### 📝 文档变更

- **README 双语支持**：添加 README 中英文双语支持
...
- **修复聊天页面按钮重复**：修复聊天页面出现两个「新建聊天」按钮的问题
