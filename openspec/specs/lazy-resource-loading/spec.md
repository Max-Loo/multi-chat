...
提供通用的按需资源加载能力，支持动态导入、缓存管理、并发控制、错误重试和 LRU 淘汰。该能力不仅限于供应商 SDK 加载，还可被其他模块复用（如图片处理库、数据可视化库等）。

**核心目标**：
- 减少应用初始加载体积（约 125KB gzipped）
- 提升应用启动速度和用户体验
- 提供通用可复用的资源加载基础设施

**适用场景**：
- 供应商 SDK 按需加载
- 大型库的延迟加载（图片处理、数据可视化等）
- 资源预加载和缓存管理
...
系统 MUST 在淘汰资源时，同时清理 `cache`、`states`、`loadingPromises` 和 `lruList` 中的记录。
...
- **AND** 系统从 `loadingPromises` 中删除该资源的 Promise
