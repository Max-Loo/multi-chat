...
定义聊天消息的操作功能规格，包括复制消息内容、编辑最新用户消息、重新生成 AI 回复、编辑历史翻页浏览，以及多模型并发场景下的数据一致性保障。
...
系统 SHALL 允许用户重新生成对话中最后一条 AI 助手回复。

#### Scenario: 重新生成最后一条 AI 回复
- **WHEN** 用户点击最后一条 AI 助手消息上的重新生成按钮
- **THEN** 系统通过位置索引，对所有 ChatModel 的 AI 回复消息，将旧 content 和 reasoningContent push 进数组，追加空字符串占位
...
#### Scenario: 重新生成失败时回滚
- **WHEN** AI 回复重新生成失败
- **THEN** 系统调用 `rollbackRegenerate` 回滚 AI 回复的 content/reasoningContent 数组
- **AND** 弹出占位空字符串，恢复到重新生成前的状态

---

### Requirement: 编辑历史翻页浏览

系统 SHALL 允许用户翻页浏览消息的编辑历史。

#### Scenario: 浏览有历史的消息
- **WHEN** 消息的 `content` 为 `string[]`（有编辑历史）
- **THEN** 气泡显示当前版本内容（数组最后一个元素）
- **AND** 显示翻页指示器（如"2/3"表示当前查看第 2 版，共 3 个版本）

#### Scenario: 切换历史版本
- **WHEN** 用户点击翻页控件
- **THEN** 气泡内容切换为对应下标的内容
- **AND** 用户消息和 AI 回复显示同一下标的版本（成对展示）
