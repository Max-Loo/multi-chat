# Remote Model Fetch Capability Specification
...
系统 MUST 使用 `providerKey` 进行精确匹配，只有白名单中的供应商才会被注册到系统中。
...
#### Scenario: 白名单为空时不注册任何供应商

- **WHEN** `ALLOWED_MODEL_PROVIDERS` 定义为空数组 `[]`
- **THEN** 系统过滤掉所有供应商
- **AND** 不注册任何 Provider 实例
- **AND** 应用进入"无可用供应商"状态
...
系统 MUST 在 `modelProviderSlice` 中定义 `loading`、`error`、`lastUpdate` 状态字段。
...
- **AND** 请求成功后，`loading` 设置为 `false`
- **AND** `lastUpdate` 设置为当前时间（ISO 8601 格式）
- **AND** 请求失败后，`loading` 设置为 `false`
- **AND** `error` 设置为错误消息字符串
