...
`setup.ts` 必须（MUST）提供 `createMockAIProvider(providerName)` 工厂函数，用于生成 AI SDK provider mock 对象。`createDeepSeek`、`createMoonshotAI`、`createZhipu` 三个 `vi.mock()` 调用必须（MUST）使用此工厂函数，仅传入 `provider` 字符串参数。

#### Scenario: 三个 provider mock 使用同一工厂

- **WHEN** `setup.ts` 中的 `@ai-sdk/deepseek`、`@ai-sdk/moonshotai`、`zhipu-ai-provider` 三个 `vi.mock()` 使用 `createMockAIProvider` 工厂
