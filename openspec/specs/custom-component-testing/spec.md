# Spec: Custom Component Testing

本规格定义了自定义通用组件的测试要求，确保独立 UI 组件的正确性和用户体验。

## Purpose

确保自定义通用组件（AnimatedLogo、FilterInput、OpenExternalBrowserButton、ProviderLogo、Skeleton 系列等）具有完整的单元测试覆盖，验证组件渲染、用户交互、边界情况和响应式行为的正确性。
