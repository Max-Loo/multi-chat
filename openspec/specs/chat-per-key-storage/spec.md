...
系统 SHALL 将每个聊天以独立 key（`chat_<id>`）存储在 Store 中，替代原有的单一 `chats` key 全量数组存储。`saveChatAndIndex` 和 `deleteChatFromStorage` 函数 SHALL 合并磁盘刷写操作，使用单次 `init()` + 批量 `set()` + 单次 `save()`，SHALL NOT 为每个子操作分别调用 `save()`。
