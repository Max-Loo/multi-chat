/**
 * useT mock 工厂函数（对应原 react-i18next mock）
 *
 * 用于 vi.mock('@/composables/useT') 的统一 mock 创建。
...
/**
 * 高频默认翻译资源（从历史测试文件中提取的公共键）
 *
 * 包含 3+ 个文件共享的翻译键，用于减少 vi.mock('@/composables/useT') 的样板代码。
 ...
 * @returns vi.mock('@/composables/useT') 的返回值
 *
 * @example
 * ```typescript
 * // 使用默认翻译
 * vi.mock('@/composables/useT', () => globalThis.__mockI18n());
 *
 * // 添加/覆盖自定义翻译
 * vi.mock('@/composables/useT', () => globalThis.__mockI18n({ setting: { key: '自定义' } }));
 * ```
 */
...
export function createI18nMockReturn<T extends Record<string, unknown>>(zhResources: T) {
  return {
    useT: () =>
      ((keyOrSelector: string | ((resources: T) => string), options?: Record<string, unknown>) => {
        let result = typeof keyOrSelector === 'function'
          ? keyOrSelector(zhResources)
          : (keyOrSelector as string).split('.').reduce((o: unknown, p: string) => (o as Record<string, unknown> | undefined)?.[p], zhResources as unknown) || keyOrSelector;
        if (options && typeof result === 'string') {
          Object.entries(options).forEach(([key, val]) => {
            result = (result as string).replace(`{{${key}}}`, String(val));
          });
        }
        return result;
      }) as unknown,
