...
import '@/utils/platform/keyringMigration';

/**
 * Keyring 迁移模块测试套件
 *
 * 测试 src/utils/platform/keyringMigration.ts 模块的功能
 * 覆盖 V1 → V2 迁移的各种场景
 */
describe('Keyring 迁移模块测试套件', () => {
  ...
  describe('4.1 基础功能测试', () => {
    it('应该正确导出迁移函数', async () => {
      const { migrateKeyringV1ToV2, isMigrationToV2Complete } = await import('@/utils/platform/keyringMigration');
    ...
    it('应该正确导出密钥派生函数', async () => {
      const { deriveEncryptionKeyV1, deriveEncryptionKeyV2 } = await import('@/utils/platform/keyringMigration');
    ...
    it('应该导出正确的版本常量', async () => {
      const { KEYRING_VERSION_KEY, KEYRING_CURRENT_VERSION } = await import('@/utils/platform/keyringMigration');
  ...
  describe('4.2 测试场景：新用户无迁移', () => {
    it('应该在没有种子时跳过迁移', async () => {
      const { migrateKeyringV1ToV2 } = await import('@/utils/platform/keyringMigration');
    ...
    it('应该设置版本标记为 "2"', async () => {
      const { migrateKeyringV1ToV2, KEYRING_VERSION_KEY } = await import('@/utils/platform/keyringMigration');
  ...
  describe('4.3 测试场景：已迁移用户跳过迁移', () => {
    it('应该在版本为 "2" 时跳过迁移', async () => {
      const { migrateKeyringV1ToV2, KEYRING_VERSION_KEY, KEYRING_CURRENT_VERSION } = await import('@/utils/platform/keyringMigration');
    ...
    it('isMigrationToV2Complete 应该返回 true', async () => {
      const { isMigrationToV2Complete, KEYRING_VERSION_KEY } = await import('@/utils/platform/keyringMigration');
  ...
  describe('4.4 测试场景：V1 → V2 成功迁移', () => {
    it('应该成功迁移使用 V1 密钥加密的数据', async () => {
      const {
        ...
        KEYRING_VERSION_KEY
      } = await import('@/utils/platform/keyringMigration');
  ...
  describe('4.8 测试场景：新密钥派生方式（仅 seed）加密/解密一致性', () => {
    it('V2 密钥派生应该只依赖 seed', async () => {
      const { deriveEncryptionKeyV2 } = await import('@/utils/platform/keyringMigration');
  ...
  describe('4.9 测试场景：密钥派生不依赖 userAgent', () => {
    it('V1 密钥派生应该依赖 userAgent', async () => {
      const { deriveEncryptionKeyV1 } = await import('@/utils/platform/keyringMigration');
  ...
  describe('变异测试补强 - PBKDF2 参数传递', () => {
    it('deriveKey iterations 等于 getPBKDF2Iterations()', async () => {
      const { deriveEncryptionKeyV2 } = await import('@/utils/platform/keyringMigration');
      const { getPBKDF2Iterations } = await import('@/utils/platform/env');
  ...
  describe('变异测试补强 - 迁移成功后数据完整性', () => {
    it('迁移成功后 IndexedDB 记录能被 V2 密钥解密', async () => {
      ...
      const { encrypt: cryptoEncrypt, decrypt: cryptoDecrypt } = await import('@/utils/platform/crypto-helpers');
    ...
    it('迁移成功后 keyring.resetState 被调用', async () => {
      const { WebKeyringCompat } = await import('@/utils/platform/keyring');
      const resetSpy = vi.spyOn(WebKeyringCompat.prototype, 'resetState');

      const { migrateKeyringV1ToV2, deriveEncryptionKeyV1 } = await import('@/utils/platform/keyringMigration');
      const { encrypt: cryptoEncrypt } = await import('@/utils/platform/crypto-helpers');
  ...
  describe('变异测试补强 - isMigrationToV2Complete localStorage 异常', () => {
    it('localStorage.getItem 抛异常时返回 false', async () => {
      const { isMigrationToV2Complete } = await import('@/utils/platform/keyringMigration');
