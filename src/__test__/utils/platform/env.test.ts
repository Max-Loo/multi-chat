...
let envModule: typeof import('@/utils/platform/env');

beforeAll(async () => {
  // 通过 vi.importActual 导入真实模块，绕过 setup/mocks.ts 中的全局 mock
  envModule = await vi.importActual<typeof import('@/utils/platform/env')>('@/utils/platform/env');
});
  ...
  describe('getPBKDF2Iterations', () => {
    ...
    it('应该返回 100000 在非测试环境中', async () => {
      ...
      try {
        ...
        vi.resetModules();
        const freshModule = await vi.importActual<typeof import('@/utils/platform/env')>('@/utils/platform/env');
