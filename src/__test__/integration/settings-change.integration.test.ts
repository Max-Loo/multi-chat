...
describe('设置变更集成测试', () => {
  ...
  describe('语言切换流程测试', () => {
    it('用户切换语言 → Redux 更新 → i18next 更新 → localStorage 持久化', async () => {
    ...
    it('验证 setAppLanguage action 触发', () => {
  ...
  describe('推理内容开关流程测试', () => {
    it('用户切换开关 → Redux 更新 → localStorage 持久化', async () => {
    ...
    it('验证 setTransmitHistoryReasoning action 触发', () => {
      const spy = vi.spyOn(testStore, 'dispatch');
      testStore.dispatch(setTransmitHistoryReasoning(true));
      expect(spy).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('验证 localStorage 保存', async () => {
      testStore.dispatch(setTransmitHistoryReasoning(true));
      await waitForLocalStorage(LOCAL_STORAGE_TRANSMIT_HISTORY_REASONING_KEY, 'true');
    });

    it('验证 chatService 应用设置', () => {
      testStore.dispatch(setTransmitHistoryReasoning(true));
  ...
  describe('自动命名开关流程测试', () => {
    ...
    it('验证 setAutoNamingEnabled action 触发', () => {
      const spy = vi.spyOn(testStore, 'dispatch');
      testStore.dispatch(setAutoNamingEnabled(false));
      expect(spy).toHaveBeenCalledWith(expect.objectContaining({
  ...
  describe('自动命名开关持久化测试', () => {
    ...
    it.each([
      ['true', true],
      ['false', false],
    ] as const)('localStorage 为 %s 时恢复为 %s', async (storedValue: string, expected: boolean) => {
