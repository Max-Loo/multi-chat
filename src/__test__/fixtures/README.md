...
### 4. Store Fixtures (`helpers/mocks/testState.ts`)

Redux 状态的工厂函数，用于创建测试用的 Redux state。

**注意**：这些 Fixtures 位于 `src/__test__/helpers/mocks/testState.ts`，可以通过 `@/__test__/helpers/mocks` 导入。

**可用函数**：

- `createChatSliceState(overrides?)` - 创建 Chat slice 状态
- `createChatPageSliceState(overrides?)` - 创建 ChatPage 状态
- `createAppConfigSliceState(overrides?)` - 创建应用配置状态
- `createModelProviderSliceState(overrides?)` - 创建模型供应商状态
- `createTestRootState(overrides?)` - 创建完整的 Redux 根状态
