/**
 * Moonshot AI (Kimi) API MSW Handlers
 * 模拟 Kimi API 的各种响应场景
 */
...
export const kimiHandlers: ApiHandlerFactory = {
  /**
   * 成功场景：返回流式响应
   * @example
   * server.use(kimiHandlers.success({ response: customStream }));
   * server.use(kimiHandlers.success({ delay: 1000 }));
   */
  success: (options: StreamOptions = {}) =>
    http.post('https://api.moonshot.cn/v1/chat/completions', async () => {
      ...
      const stream = response ?? createDefaultStream('你好！我是 Kimi。');
   ...
   * 超时场景：模拟 API 请求超时
   * @example
   * server.use(kimiHandlers.timeout({ delay: 30000 }));
   */
   ...
   * 服务器错误场景：模拟 5xx 错误
   * @example
   * server.use(kimiHandlers.serverError({ status: 500, message: 'Internal Server Error' }));
   */
  serverError: (
...
export const kimiHandlersList = [kimiHandlers.success()];
