...
const ModelConfigForm: React.FC<ModelConfigFormProps> = ({
  ...
  const currentProvider = useSelector((state: RootState) =>
    state.modelProvider.providers.find(
      (p) => p.providerKey === modelProviderKey,
    ),
  );
  ...
  const [isModelEnable, setIsModelEnable] = useState(
    isBoolean(modelParams?.isEnable) ? modelParams.isEnable : true,
  );
  ...
  const getFullModelParams = (manualConfig: ManualConfigModel): Model => {
    ...
    return {
      ...
      modelName:
        defaultModelList.find(
          (item: { modelKey: string; modelName: string }) => {
        ...
        >
          <form.Field
            ...
            children={(field) => (
              <FormItem
                field={field}
                className="w-full grow xl:w-[calc(50%-16px)]"
              >
            ...
            validators={{
              onChange: ({ value }) => {
                const result = z
                  .string()
                  .trim()
                  .min(1, { message: t(($) => $.model.apiKeyRequired) })
                  .safeParse(value);
                return result.success
                  ? undefined
                  : result.error.issues[0]?.message;
            ...
            validators={{
              onChange: ({ value }) => {
                const result = z
                  .string()
                  .trim()
                  .min(1, { message: t(($) => $.model.modelRequired) })
                  .safeParse(value);
                return result.success
                  ? undefined
                  : result.error.issues[0]?.message;
